...
class CashDrawerControllerTest {
    ...
    @InjectMocks private CashDrawerController controller;
    ...
    void setUp() {
        ...
        drawer = CashDrawer.builder().id(1L).restaurant(r).drawerName("Main").isActive(true).build();
    ...
    @Test @DisplayName("POST / — create") void create() throws Exception {
        CreateCashDrawerRequest req = new CreateCashDrawerRequest(); req.setDrawerName("Main");
        when(cashDrawerService.createCashDrawer(eq(1L), any())).thenReturn(drawer);
    ...
    @Test @DisplayName("POST /{id}/open") void open() throws Exception {
        when(cashDrawerService.openDrawer(eq(1L), eq(1L), any(), any())).thenReturn(operation);
        mockMvc.perform(post(BASE + "/1/open").param("operatorId", "1")).andExpect(status().isOk());
    }
    @Test @DisplayName("POST /{id}/paid-in") void paidIn() throws Exception {
        when(cashDrawerService.recordPaidIn(eq(1L), any(), eq(1L), any(), any())).thenReturn(operation);
        mockMvc.perform(post(BASE + "/1/paid-in").param("amount", "100000").param("operatorId", "1"))
                .andExpect(status().isOk());
    }
    @Test @DisplayName("POST /{id}/paid-out") void paidOut() throws Exception {
        when(cashDrawerService.recordPaidOut(eq(1L), any(), eq(1L), any(), any())).thenReturn(operation);
        mockMvc.perform(post(BASE + "/1/paid-out").param("amount", "50000").param("operatorId", "1"))
                .andExpect(status().isOk());
    }
    @Test @DisplayName("POST /{id}/drop") void drop() throws Exception {
        when(cashDrawerService.recordCashDrop(eq(1L), any(), eq(1L), any(), any())).thenReturn(operation);
        mockMvc.perform(post(BASE + "/1/drop").param("amount", "200000").param("operatorId", "1"))
                .andExpect(status().isOk());
    }
    @Test @DisplayName("POST /{id}/pickup") void pickup() throws Exception {
        when(cashDrawerService.recordCashPickup(eq(1L), any(), eq(1L), any(), any())).thenReturn(operation);
        mockMvc.perform(post(BASE + "/1/pickup").param("amount", "100000").param("operatorId", "1"))
                .andExpect(status().isOk());
    }
    @Test @DisplayName("POST /{id}/close") void close() throws Exception {
        ...
        when(cashDrawerService.closeDrawer(eq(1L), eq(1L), any(), any())).thenReturn(result);
        mockMvc.perform(post(BASE + "/1/close").param("operatorId", "1").param("countedAmount", "700000"))
                .andExpect(status().isOk());
    }
    @Test @DisplayName("GET /{id}/status") void status() throws Exception {
        ...
        when(cashDrawerService.getDrawerStatus(eq(1L), any())).thenReturn(resp);
        mockMvc.perform(get(BASE + "/1/status")).andExpect(status().isOk());
    }
    @Test @DisplayName("GET /{id}/history") void history() throws Exception {
        when(cashDrawerService.getOperationHistory(eq(1L), any()))
                .thenReturn(new PageImpl<>(List.of(), PageRequest.of(0, 20), 0));
        mockMvc.perform(get(BASE + "/1/history")).andExpect(status().isOk());
    }
    @Test @DisplayName("GET /shifts/{id}/operations") void shiftOps() throws Exception {
